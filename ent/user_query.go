...
func (uq *UserQuery) loadRecords(ctx context.Context, query *RecordQuery, nodes []*User, init func(*User), assign func(*User, *Record)) error {
	...
	for _, n := range neighbors {
		fk := n.user_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
