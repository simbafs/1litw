...
func (rq *RecordQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Record, init func(*Record), assign func(*Record, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_records == nil {
			continue
		}
		fk := *nodes[i].user_records
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
